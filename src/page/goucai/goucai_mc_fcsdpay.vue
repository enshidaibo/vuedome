<template>
	<div>
		...
		<header class="aui-bar aui-bar-nav">
			...
			<div>
				<div class="aui-title" tapmode>福彩3D
				<span v-if="pay3d.type=='1'" class="type">直选</span>
				<span v-else-if="pay3d.type=='3'" class="type">组三</span>
				<span v-else-if="pay3d.type=='6'" class="type">组六</span>
				</div>
			</div>
		...
		<div id="mc_fcsdpay">
			...
			<div class="ssqpay plspay" v-if='pay3d.data'>
				<!-- <div class="cont">
      <div class="shanc"><img src="../image/frame1/jjc/close_btn.png" /> </div>
      <div class="xuanh">
        <div class="hao">
          <span>1</span><span>2</span><span>3</span><span>4</span><span>5</span><span>6</span><span>7</span><span>8</span><span>9</span><i>|</i>
          <span>1</span><span>2</span><span>3</span><span>4</span><span>5</span><span>6</span><span>7</span><span>8</span><span>9</span><i>|</i>
          <span>1</span><span>2</span><span>3</span><span>4</span><span>5</span><span>6</span><span>7</span><span>8</span><span>9</span><i>|</i>
        </div>
        <p><span>直选<i> 1,000 </i>注</span><span>共<i> 2,000 </i>元</span></p>
      </div>
      <div class="jiant"></div>
    </div> -->
				<div class="" v-if='pay3d.type=="1"'>
					<div class="cont" v-for="(item,index) in pay3d.data">
						...
						<div class="xuanh">
							<div class="hao" @click="addset(index)">
								<span v-for="(c,cindex) in item.baiweiarr">{{c}}</span>|<span v-for="(d,dindex) in item.shiweiarr">{{d}}</span>|<span
				...
				<div class="" v-if='pay3d.type=="3"'>
					<div class="cont" v-for="(item,index) in pay3d.data">
						<div class="shanc" @click="closepaydata(index)"><img src="../../assets/goucai/jjc/close_btnre.png" /> </div>
						<div class="xuanh" @click="addset(index)">
							<div class="hao">
								<span v-for="(d,dindex) in item.zuSanarr">{{d}}</span>
							</div>
							<p><span>组三<i> {{item.totalNum}} </i>注</span><span>共<i> {{item.totalNum*2}} </i>元</span></p>
						</div>
						<div class="jiant" @click="addset(index)"></div>
					</div>
				</div>
				<div class="" v-if='pay3d.type=="6"'>
					<div class="cont" v-for="(item,index) in pay3d.data">
						<div class="shanc" @click="closepaydata(index)"><img src="../../assets/goucai/jjc/close_btnre.png" /> </div>
						<div class="xuanh" @click="addset(index)">
							<div class="hao">
								<span v-for="(d,dindex) in item.zuLiuarr">{{d}}</span>
							</div>
							<p><span>组六<i> {{item.totalNum}} </i>注</span><span>共<i> {{item.totalNum*2}} </i>元</span></p>
						</div>
						<div class="jiant" @click="addset(index)"></div>
					</div>
				</div>

				<!-- <div class="cont" v-if='pay3d.data[0].playType="group3"'>
      <div class="shanc"><img src="../image/frame1/jjc/close_btn.png" /> </div>
      <div class="xuanh" v-for="(item,index) in pay3d.data">
        <div class="hao">
          <span v-for="(d,dindex) in item.zuLiuarr">{{d}}</span>
          <p><span>组三 <i> {{item.totalNum}}</i>注</span><span>共<i> {{item.totalNum*2}} </i>元</span></p>
      </div>
      ...
      <div class="jiant"></div>
				...
				<div class="bottom">
					...
					<div v-if='qishu > 1' class="tingz"><span @click="xuanZ" class="xuanz" :class="{active:xuanz}"></span>中奖后停止追号</div>
					<!-- 				<footer>
		...
		<module-pay :payPopupP='payPopup' :payMoneyP='payMoney' :coinsMoneyP='coinsMoney' :cashMoneyP='cashMoney'
		 :opnenWeixinP='opnenWeixin' :opnenBankopenP='opnenBankopen' :hasHongbaoP='false' :typeName='"福彩3D"' @payPopupC="payPopupC"
...
<script>
	...
	export default {
		...
		created() {
			...
			this.noteNum = num;
			if (this.pay3d.data[0].playType == 'group3') {
				if (this.pay3d.data[0].totalNum == 2) {
					caiPiao = 'unitary'
				} else {
					caiPiao = 'doubleEntry'
				}
			} else if (this.pay3d.data[0].totalNum == 1) {
				caiPiao = 'unitary'
			} else {
				caiPiao = 'doubleEntry'
			}
		...
		methods: {
			...
			open_hemai: function() {
				...
				this.$router.push({
					...
					query: {
						'pageName': "number/lows/welfare3d",
						'Fnumber': Fnumber,
						payid: 'welfare3d',
			...
			randomone: function() {
				...
				} else if (this.pay3d.type == 3) {
					var zuSanarr = [];
					zuSanarr = this.generateRandom(zuSanarr, 2);
					var totalNum = 2;
					onedata = {
						playMethod: "normal",
						playType: "group3",
				...
				} else if (this.pay3d.type == 6) {
					var zuLiuarr = [];
					zuLiuarr = this.generateRandom(zuLiuarr, 3);
					var totalNum = 1;
					onedata = {
						playMethod: "normal",
						playType: "group6",
			...
			setData: function() {
				...
				var content = [];
				for (var i = 0; i < this.pay3d.data.length; i++) {
					if (this.pay3d.data[i].playType == 'direct') {
						var betStr = this.pay3d.data[i].baiweiarr.join(",") + "|" + this.pay3d.data[i].shiweiarr.join(",") + "|" +
							this.pay3d.data[i].geweiarr.join(",");
					} else if (this.pay3d.data[i].playType == 'group3') {
						var betStr = this.pay3d.data[i].zuSanarr.join(",")
					} else if (this.pay3d.data[i].playType == 'group6') {
						var betStr = this.pay3d.data[i].zuLiuarr.join(",")
					}
					let strdata = [];
