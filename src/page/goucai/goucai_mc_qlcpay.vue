...
<!-- 		<header class="aui-bar aui-bar-nav">
		 ...
		 <header class="aui-bar aui-bar-nav">
		    ...
		    <div>
		      <div class="aui-title"  tapmode>七乐彩
...
<!-- 				<footer>
		...
		<module-pay :sqishu='qishu' :payPopupP='payPopup' :payMoneyP='payMoney' :coinsMoneyP='coinsMoney' :cashMoneyP='cashMoney'
		 :opnenWeixinP='opnenWeixin' :opnenBankopenP='opnenBankopen' :hasHongbaoP='true' :typeName='"七乐彩"' @payPopupC="payPopupC" @RouteLeaveC='RouteLeaveC'
		...
		methods: {
		  ...
		  hongbaogzbox:function(msg){
			...
			open_hemai: function() {
				...
				this.RouteLeave = true;
				console.log(this.RouteLeave)
				this.$router.push({
					path: '/goucai/goucai_hemai',
					query: {
						'pageName': "number/lows/seven",
						'Fnumber': Fnumber,
						payid: 'seven',
			...
			randomone: function() {
				...
				if (this.playType == 'normal') {
					...
					if (newRedArr.length == 0) {
						while (newRedArr.length < 7) {
							var num = parseInt(Math.random() * 29 + 1);
			...
			setData: function() {
				...
				for (var i = 0; i < betArr.length; i++) {
					...
					if (this.playType == 'normal') {
						...
						obj.playType = 'duplex';
						obj.betContent = betArr[i].redTuoArr;
						obj.betContentDan = betArr[i].redDanArr;
						obj.schemeUnits = betArr[i].totalNum;
						content.push({...obj});
					}
				}
