...
<header class="aui-bar aui-bar-nav">
    ...
    <div>
        <div style="width:60%;left:4rem" class="aui-title" @click="ChoiceType()"><span>江苏快三</span> <span id="pagetitle">{{playTypeName}}</span> <img src="../../assets/goucai/jjc/xiajiantou.png" /></div>
    </div>
...
<div id="gpc_hbks" v-cloak>
                ...
                <div class="battle_ks xuan_h">
                    <ul class="xuanh">
                        <template v-for="(item,index) in xuanHaoList">
                            <li :class="{ 'active': item.state }" @click="setxuanHaoList(index)"><span>{{ item.id }}</span></li>
                        </template>
                    </ul>
                </div>
            </div>
    ...
    <!-- 未实名弹窗end -->
		...
		<div class="ft_money flr">
			<p>合计金额：<span>¥{{(totalPrice*1)|numberFormat}}</span></p>
