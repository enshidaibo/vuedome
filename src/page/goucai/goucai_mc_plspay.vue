...
<!-- 		<header class="aui-bar aui-bar-nav">
		 ...
		 <header class="aui-bar aui-bar-nav">
		    ...
		    <div>
		      <div class="aui-title"  tapmode>排列三<span class="type">{{pay3d.type}}</span></div>
		    </div>
		    <div class="aui-pull-right aui-btn top-more" @click="open_hemai()" tapmode>
		      发起合买
		    </div>
		</header>
		<div id="mc_plspay" v-cloak>
			<!-- 顶部期数 -->
			...
			<!-- 内容 -->
			<div class="ssqpay plspay">

				<div class="" v-if='pay3d.type=="直选"'>
				...
				<div class="" v-if='pay3d.type=="和值"'>
					<div class="cont" v-for="(item,index) in pay3d.data">
						<div class="shanc" @click="closepaydata(index)"><img src="../../assets/goucai/jjc/close_btnre.png" /> </div>
						<div class="xuanh" @click="addset(index)">
							<div class="hao">
								<span v-for="(d,dindex) in item.heZhiarr">{{d}}</span>
							</div>
							<p><span>和值<i> {{item.totalNum}} </i>注</span><span>共<i> {{item.totalNum*2}} </i>元</span></p>
						</div>
						<div class="jiant" @click="addset(index)"></div>
					</div>
				</div>
...
<!-- 				<footer>
		...
		<module-pay :payPopupP='payPopup' :payMoneyP='payMoney' :coinsMoneyP='coinsMoney' :cashMoneyP='cashMoney'
		 :opnenWeixinP='opnenWeixin' :opnenBankopenP='opnenBankopen' :hasHongbaoP='false' :typeName='"排列三"' @payPopupC="payPopupC" @RouteLeaveC='RouteLeaveC'
...
<script>
	...
	export default {
		...
		methods: {
			...
			randomone: function() {
				...
				// 	 return
				// }
				if (this.pay3d.type == "和值") {
					Toast('该玩法不支持随机选注！')
					return false;
				}
				var onedata = {};
				if (this.pay3d.type == '直选') {
					var baiwei = [];
				...
				} else if (this.pay3d.type == "组六") {
					var zuLiuarr = [];
					zuLiuarr = this.generateRandom(zuLiuarr, 3);
					console.log(zuLiuarr);
					var totalNum = 1;
					onedata = {
						playMethod: "normal",
						playType: "group6",
    ...
    console.log(JSON.stringify(this.pay3d.data));
    for (var i = 0; i < this.pay3d.data.length; i++) {
        if (this.pay3d.data[i].playType == 'p3Direct' && this.pay3d.data[i].playMethod == 'normal') {
            var betStr = this.pay3d.data[i].baiweiarr.join(",") + "|" + this.pay3d.data[i].shiweiarr.join(",") + "|" + this.pay3d.data[i].geweiarr.join(",");
        ...
        } else if (this.pay3d.data[i].playMethod == 'sum') {
            var betStr = this.pay3d.data[i].heZhiarr.join(",")
        }
       let strdata =[];
