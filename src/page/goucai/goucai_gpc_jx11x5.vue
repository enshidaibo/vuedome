...
<header class="aui-bar aui-bar-nav">
    ...
    <div>
        <div class="aui-title" @click="ChoiceType()"><span>江西11选5</span> <span id="pagetitle">{{navname}}{{playTypeName}}</span> <img src="../../assets/goucai/jjc/xiajiantou.png" /></div>
    </div>
...
<div id="gpc_jx11x5" v-cloak>
    ...
    <div class="mc_content">
        ...
        <div v-if='title==true' class="top_mc">
            ...
            <div class="fgr">投注剩余时间:<span></span><span>{{surTimeminutes}}</span>分<span>{{surTimeseconds}}</span>秒</div>
...
var EventUtil = {
    ...
    listenTouchDirection: function (target) {
        ...
        function handleTouchEvent(event) {
            ...
            var gpc_jx11x5 = document.querySelector("#gpc_jx11x5");
            switch (event.type){
                case "touchstart":
                    startY = event.touches[0].pageY;
                      scrollTops= gpc_jx11x5.scrollTop
                    break;
                case "touchend":
                    var spanY = event.changedTouches[0].pageY - startY;
                    if(spanY > 50 && scrollTops < 3){
                        lastTen.style.height = "16rem";
                        gpc_jx11x5.style.overflow = "hidden";
						xiedajian(0);

                    } else if (spanY < -50) {
                        lastTen.style.height = "0";
                        gpc_jx11x5.removeAttribute("style");
						xiedajian(1);
                    }
                    break;
            }
...
mounted() {
	this.gpc_jx11x5 = document.querySelector("#gpc_jx11x5");
...
methods:{
	...
	changebg:function(){
		...
		var lastTen = document.querySelector(".last_ten");
		var gpc_hbks = document.querySelector("#gpc_jx11x5");
		if(this.bgzhuanhuan){
...
toPlay(){   //  玩法
        ...
        .then(res=>{
            ...
            if(res.data.status==200){
                ...
                if(content.code==100000){
                  ...
                  EventUtil.listenTouchDirection(document.getElementById("gpc_jx11x5"));
                }
            }
        })
        .catch(err=>{
    ...
    ChoiceType: function(){   //点击类型
        if (this.addset.length>0||this.addlist.length>0) {
            ...
            this.gpc_jx11x5.removeAttribute("style");
			this.bgzhuanhuan=false;
        }
    },
