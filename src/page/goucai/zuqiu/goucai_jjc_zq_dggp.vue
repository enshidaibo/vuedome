<template>
	...
	<section class="aui-refresh-content">
		...
		<div class="main" >
			<!-- 单关固赔 -->
			<div class="jjc_sc_cont jjc_hhgg_cont jjc_dggp_cont">
						...
						<div v-for="(thal,rindex) in item.matchList" :key='rindex' class="con" v-if="$options.filters.gamesShow(thal.gameName)">
							...
							<div class="fgr battle">
								...
								<div class="battle_shel">
									<table v-if='thal.chooseType1||thal.chooseType2||thal.chooseType3||thal.chooseType4||thal.chooseType5'>
										<tr v-if='thal.playTypeSingle.SPF'>
											<td>0</td>
											...
											<td rowspan="2" class="tdlast" :class="{'el-yellow':thal.moreType}" @click="showPopup('Popuphhgg',cindex,rindex)"
											 tapmode>更多<br />选项</td>
										</tr>
										<tr v-else-if='!thal.playTypeSingle.SPF'>
											<td>0</td>
											<td colspan='3' class="w-100">暂未开放</td>
											<td rowspan="2" class="tdlast" :class="{'el-yellow':thal.moreType}" @click="showPopup('Popuphhgg',cindex,rindex)"
											 tapmode>更多<br />选项</td>
										</tr>
										<tr v-if='thal.playTypeSingle.RQSPF'>
										...
										<tr v-else-if='!thal.playTypeSingle.RQSPF'>
											<td v-if='thal.handicap>0' class="c-red">主<br />+{{thal.handicap}}</td>
											<td v-else-if='thal.handicap==0' class="c-hui">主<br />{{thal.handicap}}</td>
											<td v-else>主<br />{{thal.handicap}}</td>
											<td colspan='3' class="zanweikf">暂未开放</td>
										</tr>
									</table>
									<table v-else-if='!thal.chooseType1&&!thal.chooseType2'>
										<tr>
											<td>0</td>
											<td colspan='3' class="w-100">暂未开放</td>
											<td rowspan="2" class="tdlast" :class="{'el-yellow':thal.moreType}" @click="showPopup('Popuphhgg',cindex,rindex)"
				...
				this.$ajax.post('/caipiaoshouye/index.php/home/index/getMatches',
					...
					.then(ret => {
						if (ret.status == 500) {
						    ...
						    this.getMatchSPs('SPF', matches, 1, 'DGGP'); //单关 混合过关 使用
							} else {
								alert('网络错误！');
							}
						}
				...
				this.$ajax.post('/caipiaoshouye/index.php/home/index/getMatchSPs',
					...
					.then(ret => {
						if (ret.status == 500) {
							...
							if (num == 1 && classify == 'DGGP') {
									matches = this.datalists(matches, matchSPs, playType, 1, 'DGGP'); //classify  一场制胜 单关 混合过关 需另传一次做分别
									this.getMatchSPs('RQSPF', matches, 2, 'DGGP');
								} else if (num == 2 && classify == 'DGGP') {
									matches = this.datalists(matches, matchSPs, playType, 2, 'DGGP');
									this.getMatchSPs('BQQ', matches, 3, 'DGGP');
								} else if (num == 3 && classify == 'DGGP') {
									matches = this.datalists(matches, matchSPs, playType, 3, 'DGGP');
									this.getMatchSPs('JQS', matches, 4, 'DGGP');
								} else if (num == 4 && classify == 'DGGP') {
									matches = this.datalists(matches, matchSPs, playType, 4, 'DGGP');
									this.getMatchSPs('BF', matches, 5, 'DGGP');
								} else if (num == 5 && classify == 'DGGP') {
									this.datalists(matches, matchSPs, playType, 5, 'DGGP');
								}
