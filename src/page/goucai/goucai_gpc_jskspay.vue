<template>
    ...
    <div class="share_btn aui-pull-right" @click="fenx()">
		...
		<header class="aui-bar aui-bar-nav">
			...
			<div>
				<div class="aui-title" tapmode>江苏快三
		...
		<module-pay :payPopupP='payPopup' :payMoneyP='payMoney' :coinsMoneyP='coinsMoney' :cashMoneyP='cashMoney'
		 :opnenWeixinP='opnenWeixin' :opnenBankopenP='opnenBankopen' :hasHongbaoP='false' :typeName='"江苏快三"' @payPopupC="payPopupC"
...
<script>
	...
	export default {
		...
		beforeRouteLeave(to, from, next) {
			if (this.RouteLeave || to.name == 'goucai_hemai' || this.ssqBetList.length == 0) {
				next()
				...
				for (var i = 0; i < betArr.length; i++) {
					...
					switch (index) {
						...
						case 2:
							...
							break;
						case 3:
							types = 'threeDifferent';
							str = betArr[i].redQiu.join(',');
							break;
						case 4:
							types = 'threeConnectEntire';
							str = betArr[i].redQiu.join(',');
							break;
						case 5:
							types = 'twoSameMulti';
						...
						case 6:
							types = 'twoSameOne';
							str1 = betArr[i].tongArr.join(',');
							str = betArr[i].bTongArr.join(',');
							break;
						case 7:
							types = 'twoDifferent';
							str = betArr[i].redQiu.join(',');
						...
						case 8:
							types = 'threeDifferent';
							str1 = betArr[i].danArr.join(',');
							str = betArr[i].tuoArr.join(',');
							break;
						case 9:
							types = 'twoDifferent';
							str1 = betArr[i].danArr.join(',');
