<template>
	<div id='temp'>
		...
		<header class="aui-bar aui-bar-nav">
			...
			<div>
				<div class="aui-title" tapmode>双色球
				<span v-if="datotype=='puTong'" class="type">普通</span>
		...
		<div id="mc_ssqpay">
			...
			<div class="ssqpay">
				<div class="cont" v-for='(d,i) in ssqBetList'>
					...
					<div class="xuanh" v-if='d.type=="puTong"' @click='compileNum(i)'>
		...
		<module-pay :sqishu='qishu' :payPopupP='payPopup' :opneKuaiJieP='opnenYinLian' :payMoneyP='payMoney' :coinsMoneyP='coinsMoney' :cashMoneyP='cashMoney'
		 :opnenWeixinP='opnenWeixin' :opnenBankopenP='opnenBankopen' :hasHongbaoP='true' :typeName='"双色球"' @payPopupC="payPopupC" @RouteLeaveC='RouteLeaveC'
...
<script>
	...
	export default {
		...
		beforeRouteLeave(to, from, next) {
			if (this.RouteLeave || to.name == 'goucai_hemai' || to.name == 'userinfo_recharge' || to.name == 'agreement' || to.name ==
				'login' || to.name == 'userinfo_recharge_weixin' || this.ssqBetList.length == 0) {
				next()
			} else {
				...
				} else if (caiPiao == 'unitary' && this.ssqBetList.length == 5) {
					MessageBox({
						title: '',
						message: '是否放弃本次投注？',
						showCancelButton: true,
		...
		created() {
			...
			this.ssqBetList = this.ssqBetList.concat(arr);
			console.log(JSON.stringify(this.ssqBetList))
			if (this.ssqBetList[0].totalNum == 1) {
				caiPiao = 'unitary'
			} else {
				caiPiao = 'doubleEntry'
			}
		...
		methods: {
			...
			randomone: function() {
				...
				if (caiPiao == 'unitary' && this.ssqBetList.length == 5) {
					Toast('单式注单最多5注!');
					return
				}
				...
				if (this.playType == 'normal') {
					var obj = {};
					//生成蓝色球号码
					var newBlueArr = [parseInt(Math.random() * 15 + 1)];
					//生成6个不重复的1-33的随机数
					var newRedArr = [];
					if (newRedArr.length == 0) {
						while (newRedArr.length < 6) {
							var num = parseInt(Math.random() * 32 + 1);
					...
					this.zhuNum(this.ssqBetList);
					setSession('arr', this.ssqBetList)
				} else {
					Toast('该玩法不支持随机')
				}
			},
			...
			setData: function() {
				...
				for (var i = 0; i < betArr.length; i++) {
					...
					if (this.playType == 'normal') {
						...
						obj.playType = 'duplex';
						var str = betArr[i].redDanArr.join(',') + '_' + betArr[i].redTuoArr.join(',') + '|' + betArr[i].blueQiu.join(',');
						obj.betContent = str;
						obj.schemeUnits = betArr[i].totalNum;
						content.push({...obj});
					}
