<template>
	...
	<div>
		<section class="aui-refresh-content">
			...
			<div class="daxf_fm main">
				<template v-for="(item, cindex) in RQSPFlist">
					<div class="countime" :id="'jjc_dxfTime' + cindex" @click="showList(cindex)" data-state="open">
					...
					<div class="jjc_sc_cont jjc_rxjc_cont jjc_dxf_cont" v-show="showListarr[cindex]">
						<div v-for="(thal, rindex) in item.matchList" class="con" v-if="$options.filters.gamesShow(thal.gameName)">
							...
							<div class="fgr battle">
								<div class="battle_name"><span><i class="zkdui">(客) </i>{{thal.guestTeamName}} </span><span>vs</span><span>{{thal.homeTeamName}}
										<i class="zkdui">(主)</i></span></div>
								<div class="battle_shel">
									<ul>
										<li :class="{'el-yellow': thal.soddsType}" @click="selectJJC_erxy(cindex,rindex,'soddsType','thal.id')"><span>大分</span><br /><span>{{thal.fodds}}</span></li>
										<li><span>分数</span><br /><span>{{thal.singleTotalScore}}</span></li>
										<li :class="{'el-yellow': thal.foddsType}" @click="selectJJC_erxy(cindex,rindex,'foddsType','thal.id')"><span>小分</span><br /><span>{{thal.fodds}}</span></li>
									</ul>
								</div>
							</div>
						</div>
