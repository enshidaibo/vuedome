...
<header class="aui-bar aui-bar-nav">
    ...
    <div>
        <div class="aui-title" @click="ChoiceType()"><span>山东11运夺金</span> <span id="pagetitle">{{navname}}{{playTypeName}}</span> <img src="../../assets/goucai/jjc/xiajiantou.png" /></div>
    </div>
    <div class="aui-pull-right aui-btn top-more" @click="menuPage()">
        <span></span><span></span><span></span>
    </div>
</header>
<div id="gpc_sd11ydj" v-cloak>
    <!-- 购彩类型 -->
...
var EventUtil = {
    ...
    listenTouchDirection: function (target) {
        ...
        function handleTouchEvent(event) {
            ...
            var gpcSd11ydj = document.querySelector("#gpc_sd11ydj");
            switch (event.type){
                case "touchstart":
                    startY = event.touches[0].pageY;
                    scrollTops= gpcSd11ydj.scrollTop
                    break;
                case "touchend":
                    var spanY = event.changedTouches[0].pageY - startY;
                    if(spanY > 50 && scrollTops < 3){
                        lastTen.style.height = "16rem";
                        gpcSd11ydj.style.overflow = "hidden";
						xiedajian(0);
                    } else if (spanY < -50) {
                        lastTen.style.height = "0";
                        gpcSd11ydj.removeAttribute("style");
						xiedajian(1);
                    }
                    break;
            }
    ...
    return {
        ...
        styleList: {
            'normalList': [
                ...
                {'name': '前一','limitNumber': 1,'playType': 'randomTwo','AmoMoney': '1170','typeName': '','type': 'normal','text': '从01～11中任选1个或多个号码，所选号码与开奖号码第一位号码相同，即中奖13元。'},
            ],
            'duplexList': [
...
mounted() {
	this.gpcSd11ydj = document.querySelector("#gpc_sd11ydj");
...
methods:{
	...
	changebg:function(){
		...
		var lastTen = document.querySelector(".last_ten");
		var gpc_hbks = document.querySelector("#gpc_sd11ydj");
		if(this.bgzhuanhuan){
    ...
    getHistoryDraws: function() {    //请求数据
        var data = [];
        data['flag'] = false;
        data['page'] = 1;
        data['rows'] = 10;
        var datas =  this.Qs.stringify({
            path:'number/keno/sdel11to5',data
        },{allowDots: true});
        ...
        .then(res=>{
            ...
            if(res.data.status==200){
                ...
                if(content.code==100000){
                  ...
                  EventUtil.listenTouchDirection(document.getElementById("gpc_sd11ydj"))
                }
            }
        })
        .catch(err=>{
    ...
    setSurplusTime: function() {
        ...
        if (sTm[0].toString().length<2) {this.surTimedays = '0'+sTm[0].toString();}
        else {this.surTimedays = sTm[0];}
        if (sTm[1].toString().length<2) {this.surTimehours = '0'+sTm[1].toString();}
        else {this.surTimehours = sTm[1]}
        if (sTm[2].toString().length<2) {this.surTimeminutes = '0'+sTm[2].toString();}
        else {this.surTimeminutes = sTm[2]}
        if (sTm[3].toString().length<2) {this.surTimeseconds = '0'+sTm[3].toString();}
        else {this.surTimeseconds = sTm[3];}
        var that = this;
        this.interval = setInterval(function () {
            ...
            if (Tm[0].toString().length<2) {that.surTimedays = '0'+Tm[0].toString();}
            else {that.surTimedays = Tm[0];}
            if (Tm[1].toString().length<2) {that.surTimehours = '0'+Tm[1].toString();}
            else {that.surTimehours = Tm[1]}
            if (Tm[2].toString().length<2) {that.surTimeminutes = '0'+Tm[2].toString();}
            else {that.surTimeminutes = Tm[2]}
            if (Tm[3].toString().length<2) {that.surTimeseconds = '0'+Tm[3].toString();}
            else {that.surTimeseconds = Tm[3];}
        },1000)
    },
    sanshimiao: function(time){
        ...
        this.inter = setInterval(function () {
            ...
            if (DTm[0].toString().length<2) {that.surTimedays = '0'+DTm[0].toString();}
            else {that.surTimedays = DTm[0];}
            if (DTm[1].toString().length<2) {that.surTimehours = '0'+DTm[1].toString();}
            else {that.surTimehours = DTm[1]}
            if (DTm[2].toString().length<2) {that.surTimeminutes = '0'+DTm[2].toString();}
            else {that.surTimeminutes = DTm[2]}
            if (DTm[3].toString().length<2) {that.surTimeseconds = '0'+DTm[3].toString();}
            else {that.surTimeseconds = DTm[3];}
    ...
    ChoiceType: function(){  //点击筛选类型
        if (this.addset.length>0||this.addlist.length>0) {
            ...
            this.gpcSd11ydj.removeAttribute('style');
    ...
    //     }
    // },
    hidemoc: function() {   //点击遮盖层
        this.menuStatue = false;
    },
    setClearList: function(){   //点击清除
        this.rnums=0,
    ...
    setqianWeiList: function (idx) {  // 选择
        if (this.navname=="前二直选") {
            if (this.qianWeiList[idx].state) {
                this.qianWeiList[idx].state = false
                this.snums--;
