<template>
	<div>
		<header class="aui-bar aui-bar-nav">
			...
			<div>
				<div class="aui-title" @click="ChoiceType()"><span>七乐彩</span> <span id="pagetitle">{{name}}</span> <img src="../../assets/goucai/jjc/xiajiantou.png" /></div>
			</div>
		...
		<div id="mc_qlc">
            	...
            	<div id="logok" v-show="logok">
            		<div class="renzcont">
            			<div>
                            ...
                            <p class="renz_text">认证即可获得<span>6.66</span>元现金</p>
            				<div class="logokbtn">
            ...
            <!-- 未实名弹窗end -->
				...
				<div class="ft_btn fgr">
					...
					<div class="flr" tapmode @click="open_qlcpay()">投注<span v-if="betConlength > 0" class="pay_b">{{ betConlength > 99?"99":betConlength }}</span></div>
				</div>
			</footer>
<!-- 
			<footer>
...
<script>
	...
	var EventUtil = {
		...
		listenTouchDirection: function(target) {
			...
			function handleTouchEvent(event) {
				...
				var gpc_hbks = document.querySelector("#mc_qlc");
				switch (event.type) {
					case "touchstart":
    ...
    //         },
			...
			Random: function() {
				this.redDallIdx = 0
				for (var i = 0;; i++) {
					if (this.ccarray.length < 7) {
						var rand1 = this.generateRandom(30)
						if (rand1 != null && rand1 < 30) {
				...
				if (rnums < 7) {
					...
					var totalNum = parseInt((getfactorial(rnums) / (getfactorial(rnums - 7) * getfactorial(7))));
			...
			setdanDallList: function(idx) {
				...
				if (dnums < 1 || 8 - dnums > tnums) {
					this.totalNum = 0;
					this.totalPrice = 0;
				} else {
					var totalNum = parseInt(getfactorial(tnums) / (getfactorial(7 - dnums) * getfactorial(tnums - (7 - dnums))));
					this.totalNum = totalNum;
					this.totalPrice = totalNum * 2;
				}
					...
					.then(data => {
						if (data.status == 200) {
							...
							if (content.code == 100000) {
								...
								EventUtil.listenTouchDirection(document.getElementById("mc_qlc"))
							}
						}
					})
					.catch(err => {
