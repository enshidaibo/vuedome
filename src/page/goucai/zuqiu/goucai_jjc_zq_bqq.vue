<template>
	<div>
		...
		<div class="main">
			<!-- 半全场  -->
			<div class="jjc_sc_cont jjc_bqc_cont">
						...
						<div v-for="(thal,rindex) in item.matchList" :key='rindex' class="con" v-if="$options.filters.gamesShow(thal.gameName)">
							...
							<div class="fgr battle">
								...
								<div class="battle_shel">
									<div class="tzxx" @click="showPopup('Popupbqc',cindex,rindex)" tapmode v-if="thal.chooseBet&&thal.chooseBet.length>0"><template
										 v-for='(d,i) in thal.chooseBet'>
									...
									<div class="tzxx" @click="showPopup('Popupbqc',cindex,rindex)" tapmode v-else-if='thal.erXY'>点击展开投注选项</div>
		...
		<!-- 半全场点击投注弹出窗 -->
		<div v-if="PopupArr.Popupbqc"  class="mask banquanchang"  @click="hidePopup('Popupbqc')"  @touchmove.prevent>
			</div>
			<div class="banqc_cont bqc_con" @touchmove.prevent>
				<div v-if="PopupArr.Popupbqc">
				<div class="title"><span>{{bf.homeTeamName}}</span><span> vs </span><span>{{bf.guestTeamName}}</span></div>
				<div class="con">
					<table class="bqc">
						<tr>
							<td rowspan='3' class="td1">半全场</td>
							<td :class="{ 'el-yellow': bf.chooseType[0] }" @click="selectJJC_bqc(0,'胜-胜',bf.erXY[0])">胜-胜<span>{{bf.erXY[0]}}</span></td>
							<td :class="{ 'el-yellow': bf.chooseType[1] }" @click="selectJJC_bqc(1,'胜-平',bf.erXY[1])">胜-平<span>{{bf.erXY[1]}}</span></td>
							<td :class="{ 'el-yellow': bf.chooseType[2] }" @click="selectJJC_bqc(2,'胜-负',bf.erXY[2])">胜-负<span>{{bf.erXY[2]}}</span></td>
						</tr>
						<tr>
							<td class="fs-we" :class="{ 'el-yellow': bf.chooseType[3] }" @click="selectJJC_bqc(3,'平-胜',bf.erXY[3])">平-胜<span>{{bf.erXY[3]}}</span></td>
							<td :class="{ 'el-yellow': bf.chooseType[4] }" @click="selectJJC_bqc(4,'平-平',bf.erXY[4])">平-平<span>{{bf.erXY[4]}}</span></td>
							<td :class="{ 'el-yellow': bf.chooseType[5] }" @click="selectJJC_bqc(5,'平-负',bf.erXY[5])">平-负<span>{{bf.erXY[5]}}</span></td>
						</tr>
						<tr>
							<td class="fs-we" :class="{ 'el-yellow': bf.chooseType[6] }" @click="selectJJC_bqc(6,'负-胜',bf.erXY[6])">负-胜<span>{{bf.erXY[6]}}</span></td>
							<td :class="{ 'el-yellow': bf.chooseType[7] }" @click="selectJJC_bqc(7,'负-平',bf.erXY[7])">负-平<span>{{bf.erXY[7]}}</span></td>
							<td :class="{ 'el-yellow': bf.chooseType[8] }" @click="selectJJC_bqc(8,'负-负',bf.erXY[8])">负-负<span>{{bf.erXY[8]}}</span></td>
						</tr>
					</table>
				</div>
				<div class="foot">
					<div @click="hidePopup('Popupbqc')">取消</div>
					<div @click="confirmChoice('Popupbqc')">确定</div>
				</div>
			</div>
		</div>
	</div>
...
// 		},
		...
		methods: {
			...
			selectJJC_bqc: function(idx, list, bet) {
				if (bet*1=='--') {
                Toast('不能选择暂无赔率的赛事。')
                return false;
              }
