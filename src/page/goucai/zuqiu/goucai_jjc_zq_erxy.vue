	...
	<div>
		<section class="aui-refresh-content">
			<div class="aui-content">
				...
				<div class="main">
					<!-- 二选一 -->
					<div class="jjc_sc_cont jjc_rxjc_cont jjc_exy_cont">
								...
								<div v-for="(thal,rindex) in item.matchList" :key='rindex' class="con" v-if="$options.filters.gamesShow(thal.gameName)">
									...
									<div class="fgr battle">
										<div class="battle_shel">
											<ul class="exy_ul">
												<li :class="{'el-yellow': thal.soddsTypeY}" @click="selectJJC_erxy(cindex,rindex,'soddsTypeY',thal.erXY)"><span>{{ thal.homeTeamName }}</span><br /><span
													 v-if='thal.erXY'><span v-if="thal.handicap==1">主不败{{thal.erXY[1][0]}}</span><span v-else>主胜{{thal.erXY[0][0]}}</span></span><span
													 v-else>暂未开放</span></li>
												<li :class="{'el-yellow': thal.poddsTypeY}" @click="selectJJC_erxy(cindex,rindex,'poddsTypeY',thal.erXY)"><span>{{thal.guestTeamName}}</span><br /><span
													 v-if='thal.erXY'><span v-if="thal.handicap==1">客胜{{thal.erXY[0][2]}}</span><span v-else>客不败{{thal.erXY[1][2]}}</span></span><span
													 v-else>暂未开放</span></li>
											</ul>
										</div>
									</div>
...
<script>
	...
	export default {
		...
		methods: {
						...
						for (let n = 0; n < matchSPs.length; n++) {
							if (matchekey == matchSPs[n].matchKey) {
								if (JSON.parse(matchSPs[n].content).sp[0] * 1 == 0) {
									JSON.parse(matchSPs[n].content).sp = '--';
								}
						...
						for (let y = 0; y < matches[x].matchList.length; y++) {
							for (let i = 0; i < array.length; i++) {
								if (matches[x].matchList[y].matchKey == array[i].matchKey) {
									...
									if (array[i].soddsTypeY || array[i].poddsTypeY) {
										lengths++
									}
								}
							}
			...
			selectJJC_erxy: function(cindex, rindex, key, bet) {
				if (key == 'soddsTypeY') {
					if (this.matches[cindex].matchList[rindex].handicap == '1') {
						if (this.matches[cindex].matchList[rindex].erXY[1][0] * 1 == '--') {
							Toast('不能选择暂无赔率的赛事。')
							return false;
						}
					} else {
						if (this.matches[cindex].matchList[rindex].erXY[0][0] * 1 == '--') {
							Toast('不能选择暂无赔率的赛事。')
							return false;
						}
					}
				...
				if (key == 'poddsTypeY') {
					if (this.matches[cindex].matchList[rindex].handicap == '1') {
						if (this.matches[cindex].matchList[rindex].erXY[0][2] * 1 == '--') {
							Toast('不能选择暂无赔率的赛事。')
							return false;
						}
					} else {
						if (this.matches[cindex].matchList[rindex].erXY[1][2] * 1 == '--') {
							Toast('不能选择暂无赔率的赛事。')
							return false;
						}
					}
				...
				if (matchList.soddsTypeY == true || matchList.poddsTypeY == true) {
