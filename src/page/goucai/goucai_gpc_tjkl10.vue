...
<header class="aui-bar aui-bar-nav">
    ...
    <div>
        <div class="aui-title" @click="ChoiceType()"><span>天津快乐10分</span> <span id="pagetitle">{{navname}}{{playTypeName}}</span> <img src="../../assets/goucai/jjc/xiajiantou.png" /></div>
    </div>
...
<div id="gpc_tjkl10" v-cloak>
    ...
    <div class="mc_content">
        ...
        <div v-if="danDallIdx.list == 'normalList'&&danDallIdx.index != 7&&danDallIdx.index != 9" class="mc_cont">
            ...
            <div class="yaoyiyao" @click="Random(danDallIdx.index)">
                <p>摇一摇机选</p>
            </div>
        ...
        <div v-if="danDallIdx.list == 'duplexList'" class="mc_cont mc_cont2">
            ...
            <div class="baiwei">
                <div class="titl">
                    <div>至多可选{{ styleList.duplexList[danDallIdx.index].limitNumber }}个号码</div>
                </div>
                <div class="battle battle_red">
                    <div class="battle_icon flr">胆码
        ...
        <template v-if="danDallIdx.list == 'normalList'">
            ...
            <div v-if="danDallIdx.index == 7||danDallIdx.index == 9" class="mc_cont mc_cont2">
				<span :class="bgzhuanhuan?'iconbg_up':'iconbg_down'" @click="changebg"></span>
            <div class="yaoyiyao" @click="Random(danDallIdx.index)"><p>摇一摇机选</p></div>
            ...
            <div class="shiwei">
                <div class="battle battle_red">
                    <div class="battle_icon flr" style="width: 15%;">第二位
                        <div class="right_arror"></div>
                    </div>
                    <div class="battle_shuz flr" style="width: 80%;">
                        <ul>
            ...
            <div v-if="danDallIdx.index == 9" class="gewei">
                <div class="battle battle_red">
                    <div class="battle_icon flr" style="width: 15%;">第三位
                        <div class="right_arror"></div>
                    </div>
                    <div class="battle_shuz flr" style="width: 80%;">
                        <ul>
...
<!--    <footer v-if='isShow'> 
        ...
        <div class="ft_btn fgr">
            <div class="flr" @click="setClearList()">清除</div>
            <div class="flr" @click="open_fcsdpay()">投注<span v-if="betConlength > 0" class="pay_b">{{ betConlength > 99?"99":betConlength }}</span></div>
        </div>
    </footer> -->
</div>
</div>
...
var EventUtil = {
    ...
    listenTouchDirection: function (target) {
        ...
        function handleTouchEvent(event) {
            ...
            var gpc_tjkl10 = document.querySelector("#gpc_tjkl10");
            switch (event.type){
                case "touchstart":
                    startY = event.touches[0].pageY;
                    scrollTops=gpc_tjkl10.scrollTop
                    break;
                case "touchend":
                    ...
                    if(spanY > 50 && scrollTops < 3){
                        ...
                        gpc_tjkl10.style.overflow = "hidden";
                        gpc_tjkl10.style.height = heig+"px";
						xiedajian(0);
                    } else if (spanY < -50) {
                        lastTen.style.height = "0";
                        gpc_tjkl10.removeAttribute("style");
						xiedajian(1);
                    }
                    break;
            }
...
data() {
    return {
        ...
        styleList: {
            'normalList': [
                {'name': '选一数投','limitNumber': 1,'playType': 'randomTwo','AmoMoney': '25','typeName': '普通','type': 'normal','text': '任意选1个号码，与开奖号码第一个号码相同，即中25元'},
                {'name': '选一红投','limitNumber': 1,'playType': 'randomThree','AmoMoney': '8','typeName': '普通','type': 'normal','text': '任选1个号码，若开奖号码第一个号码为19，20，即中8元'},
...
mounted() {
	this.gpc_tjkl10 = document.querySelector("#gpc_tjkl10");
...
methods:{
	...
	changebg:function(){
		...
		var lastTen = document.querySelector(".last_ten_bjk8");
		var gpc_hbks = document.querySelector("#gpc_tjkl10");
		if(this.bgzhuanhuan){
			var heig = document.documentElement.clientHeight - document.getElementsByTagName("header")[0].offsetHeight
			lastTen.style.height = "16rem";
    	...
    	.then(res=>{
    		...
    		if(res.data.status==200){
                ...
                if(content.code==100000){
                    ...
                    EventUtil.listenTouchDirection(document.getElementById("gpc_tjkl10"))
                }
            }
    	})
    	.catch(err=>{
    ...
    ChoiceType: function(){   //点击选择类型
        if(this.cantchose){
            ...
            this.gpc_tjkl10.removeAttribute("style");
			this.bgzhuanhuan=false;
        }else {
            Toast('请选择相同玩法进行投注！')
        }
