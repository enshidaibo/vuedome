	...
	<div>
		<section class="aui-refresh-content">
			<div class="aui-content">
				<div class="main mainzq" v-if="SXDSList">
					<!-- 让球胜负平 -->
					<div class="rangqspf_fm">
						<template v-for="(item, cindex) in SXDSList">
							...
							<div class="jjc_sc_cont jjc_rxjc_cont jjc_dxf_cont jjc_sxds_cont" v-show="days[cindex]">
								<div v-for="(thal,rindex) in item.matchList" class="con" v-if="$options.filters.gamesShow(thal.gameName)">
									...
									<div class="fgr battle">
										...
										<div class="battle_shel">
											<ul>
												<li :class="{ 'el-yellow': thal.stateArr.z0 }" @click="selectbjdc_sxds(cindex,rindex,'z0',thal.sp[0])"><span>上单</span><br /><span>{{thal.sp[0]}}</span></li>
												<li :class="{ 'el-yellow': thal.stateArr.z1 }" @click="selectbjdc_sxds(cindex,rindex,'z1',thal.sp[1])"><span>上双</span><br /><span>{{thal.sp[1]}}</span></li>
												<li :class="{ 'el-yellow': thal.stateArr.z2 }" @click="selectbjdc_sxds(cindex,rindex,'z2',thal.sp[2])"><span>下单</span><br /><span>{{thal.sp[2]}}</span></li>
												<li :class="{ 'el-yellow': thal.stateArr.z3 }" @click="selectbjdc_sxds(cindex,rindex,'z3',thal.sp[3])"><span>下双</span><br /><span>{{thal.sp[3]}}</span></li>
											</ul>
										</div>
									</div>
								</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			selectbjdc_sxds: function(cindex, rindex, key, sp) {
				if (sp == '--') {
					Toast("不能选择暂无赔率的赛事。");
					return false;
				}
				if(getSession('arr')){
					...
					if(arrMatchKey.indexOf(this.SXDSList[cindex].matchList[rindex].id)==-1&&arrLength==8){
						Toast('至多一次选择8场赛事。');
						return false
					}
				}
				 
				if (this.SXDSList[cindex].matchList[rindex].stateArr[key]) {
					this.SXDSList[cindex].matchList[rindex].stateArr[key] = false
				} else {
					this.SXDSList[cindex].matchList[rindex].stateArr[key] = true
				}
				for (let i = 0; i < 4; i++) {
					let key = 'z' + i
					if (this.SXDSList[cindex].matchList[rindex].stateArr[key]) {
						this.SXDSList[cindex].matchList[rindex].stateArr['z'] = 1;
						break;
					} else {
						this.SXDSList[cindex].matchList[rindex].stateArr['z'] = 0
					}
				}

				var z = 0;
				var ass = [];
				for (var i = 0; i < this.SXDSList.length; i++) {
					for (var j = 0; j < this.SXDSList[i].matchList.length; j++) {
						if (this.SXDSList[i].matchList[j].stateArr['z']) {
							z += 1;
							ass.push(this.SXDSList[i].matchList[j]);
						}
					}
				}
