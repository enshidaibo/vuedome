<template>
	<div>
		<section class="aui-refresh-content">
			...
			<div class="main" v-if="rxjcList">
				<div v-cloak class="jjc_sc_cont jjc_rxjc_cont">
					...
					<div class="" v-if='rxjcList&&days'>
						<div v-for="(thal,rindex) in rxjcList" class="con">
							<div class="flr titl">
							...
							<div class="fgr battle">
								<div class="battle_shel">
									<ul>
										  <li v-bind:class="{ 'el-yellow': thal.stateArr.b0 }" @click="selectJJC_rxjc(rindex,'b0',thal.odds1)"><span>{{ thal.homeTeamName }}</span><br />
                                              
                                                <span >胜{{ thal.odds1.toFixed(2) }}</span>
                                            </li>
                                            <li v-bind:class="{ 'el-yellow': thal.stateArr.b1 }" @click="selectJJC_rxjc(rindex,'b1',thal.odds2)"><span>VS</span><br />
                                            
                                                <span >平{{ thal.odds2.toFixed(2) }}</span>


                                            </li>
                                            <li v-bind:class="{ 'el-yellow': thal.stateArr.b2 }" @click="selectJJC_rxjc(rindex,'b2',thal.odds3)"><span>{{ thal.guestTeamName }}</span><br />
...
<script>
	...
	export default {
		...
		methods: {
			...
			selectJJC_rxjc: function(idx, key,besunm) {
				  // if (besunm*1==0) {
      //       Toast('不能选择暂无赔率的赛事。')
      //       return false;
      //     }
				if (this.rxjcList[idx].stateArr[key]) {
					this.rxjcList[idx].stateArr[key] = false
					var fhgu = 0;
					for (var i = 0; i < 3; i++) {
					  if (this.rxjcList[idx].stateArr['b' + i]) {
						fhgu++
					  }
					}
					if (fhgu == 0) {
					  ...
					  for (var i = 0; i < this.sczcBetArr.length; i++) {
						if (this.rxjcList[idx].id == this.sczcBetArr[i].id) {
						  this.sczcBetArr.splice(i,1)
						}
					  }
					}
				} else {
					this.rxjcList[idx].stateArr[key] = true
					var fhgu = 0;
					for (var i = 0; i < 3; i++) {
					  if (this.rxjcList[idx].stateArr['b' + i]) {
						fhgu++
					...
					if (fhgu == 1) {
					  this.chosenField = this.chosenField + 1
					  this.sczcBetArr.push(this.rxjcList[idx])
					}
				}
