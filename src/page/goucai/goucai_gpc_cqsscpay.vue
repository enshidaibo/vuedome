<template>
    ...
    <div class="share_btn aui-pull-right" @click="fenx()">
		...
		<header class="aui-bar aui-bar-nav">
			...
			<div>
				<div class="aui-title" tapmode>重庆时时彩
		...
		<div id="mc_ssqpay">
			...
			<div v-cloak class="bottom">
				...
				<div v-if='qishu > 1' class="tingz"><span @click="xuanZ" id="xuanz" :class="['xuanz',{'active':xuanzStatue}] "></span>中奖后停止追号</div>
				<!--        <footer>
		...
		<module-pay :payPopupP='payPopup' :payMoneyP='payMoney' :coinsMoneyP='coinsMoney' :cashMoneyP='cashMoney'
		 :opnenWeixinP='opnenWeixin' :opnenBankopenP='opnenBankopen' :hasHongbaoP='false' :typeName='"重庆时时彩"' @payPopupC="payPopupC"
	...
	export default {
		...
		beforeRouteLeave(to, from, next) {
			if (this.RouteLeave || to.name == 'goucai_hemai' || to.name == 'agreement' || this.pageParam.betContent.length == 0) {
				next()
			}else if (this.fromHome && this.fromHome == 'home') {
		...
		methods: {
			...
			setData: function() {
				...
				for (var i = 0; i < this.pageParam.betContent.length; i++) {
					...
					if (this.pageParam.danDallIdx == 0 || this.pageParam.danDallIdx == 2 || this.pageParam.danDallIdx == 4 || this.pageParam
						.danDallIdx == 5) {
						...
						betContent = betContentWan + '|' + betContentQian + '|' + betContentBai + '|' + betContentShi + '|' +
							betContentGe
					}
