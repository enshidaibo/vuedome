<template>
	<div>
		<header class="aui-bar aui-bar-nav">
			...
			<div>
				<div class="aui-title" @click="ChoiceType()"><span>重庆时时彩</span> <span id="pagetitle">{{playTypeName}}</span> <img
		...
		<div id="gpc_cqssc">
			...
			<div class="mc_content">
				...
				<div v-if="danDallIdx == 8" class="mc_cont mc_cont2">
					...
					<div class="yaoyiyao" @click="RandomDxds()">
						<p>摇一摇机选</p>
					</div>
							...
							<div class="battle_shuz flr">
								<ul>
									<template v-for="(item,index) in dxdxShiList">
										<li :class="{ 'active': item.state }" @click="setdxdxShiList(index)"><span class="sp1">{{ item.id }}</span><span
											 class="sp2" v-if="yilou">{{ redMiss.dxds.one[index] }}</span></li>
									</template>
							...
							<div class="battle_shuz flr">
								<ul>
									<template v-for="(item,index) in dxdxGeList">
										<li :class="{ 'active': item.state }" @click="setdxdxGeList(index)"><span class="sp1">{{ item.id }}</span><span
											 class="sp2" v-if="yilou">{{ redMiss.dxds.two[index] }}</span></li>
									</template>
									<!-- <li style="border: 0;"></li><li style="border: 0;"></li> -->
								</ul>
							</div>
				...
				<div v-else-if="danDallIdx == 2 || danDallIdx == 4 || danDallIdx == 5" class="mc_cont mc_cont2">
					<span :class="bgzhuanhuan?'iconbg_up':'iconbg_down'" @click="changebg"></span>
					<div v-if="danDallIdx == 4 || danDallIdx == 2" class="yaoyiyao" @click="RandomExzux()">
						<p>摇一摇机选</p>
					</div>
					<div v-else class="yaoyiyao" @click="RandomSxzl()">
						<p>摇一摇机选</p>
					</div>
					<div class="jieshao">{{ typeList[danDallIdx].text }}</div>
					<div class="gewei">
						<div class="battle battle_red">
							<div class="battle_icon flr">号码<div class="right_arror"></div>
							</div>
							<div class="battle_shuz flr">
								<ul>
									<template v-for="(item,index) in geWeiList">
										<li :class="{ 'active': item.state }" @click="setgeWeiList(index)"><span class="sp1">{{ item.id }}</span><span
											 class="sp2" v-if="yilou">{{ MissDirect[index] }}</span></li>
									</template>
									<!-- <li style="border: 0;"></li><li style="border: 0;"></li> -->
								</ul>
							</div>
				...
				<div v-else class="mc_cont mc_cont2">
					<span :class="bgzhuanhuan?'iconbg_up':'iconbg_down'" @click="changebg"></span>
					<div class="yaoyiyao" @click="Random()">
						<p>摇一摇机选</p>
					</div>
					<div class="jieshao">{{ typeList[danDallIdx].text }}</div>
							...
							<div class="battle_shuz flr">
								<ul>
									<template v-for="(item,index) in wanWeiList">
										<li :class="{ 'active': item.state }" @click="setwanWeiList(index)"><span class="sp1">{{ item.id }}</span><span
											 class="sp2" v-if="yilou">{{ MissList.one[index] }}</span></li>
									</template>
									<!-- <li style="border: 0;"></li><li style="border: 0;"></li> -->
								</ul>
							</div>
							...
							<div class="battle_shuz flr">
								<ul>
									<template v-for="(item,index) in qianWeiList">
										<li :class="{ 'active': item.state }" @click="setqianWeiList(index)"><span class="sp1">{{ item.id }}</span><span
											 class="sp2" v-if="yilou">{{ MissList.two[index] }}</span></li>
									</template>
									<!-- <li style="border: 0;"></li><li style="border: 0;"></li> -->
								</ul>
							</div>
							...
							<div class="battle_shuz flr">
								<ul>
									<template v-for="(item,index) in baiWeiList">
										<li :class="{ 'active': item.state }" @click="setbaiWeiList(index)"><span class="sp1">{{ item.id }}</span><span
											 class="sp2" v-if="yilou">{{ MissList.three[index] }}</span></li>
									</template>
									<!-- <li style="border: 0;"></li><li style="border: 0;"></li> -->
								</ul>
							</div>
							...
							<div class="battle_shuz flr">
								<ul>
									<template v-for="(item,index) in shiWeiList">
										<li :class="{ 'active': item.state }" @click="setshiWeiList(index)"><span class="sp1">{{ item.id }}</span><span
											 class="sp2" v-if="yilou">{{ MissList.four[index] }}</span></li>
									</template>
									<!-- <li style="border: 0;"></li><li style="border: 0;"></li> -->
								</ul>
							</div>
						...
						<div class="battle battle_red">
							...
							<div class="battle_shuz flr">
								<ul>
									<template v-for="(item,index) in geWeiList">
										<li :class="{ 'active': item.state }" @click="setgeWeiList(index)"><span class="sp1">{{ item.id }}</span><span
											 class="sp2" v-if="yilou">{{ MissList.five[index] }}</span></li>
									</template>
									<!-- <li style="border: 0;"></li><li style="border: 0;"></li> -->
								</ul>
							</div>
...
<script>
	...
	var EventUtil = {
		...
		listenTouchDirection: function(target) {
			...
			function handleTouchEvent(event) {
				...
				var gpc_cqssc = document.querySelector("#gpc_cqssc");
				switch (event.type) {
					case "touchstart":
						startY = event.touches[0].pageY;
						scrollTops = gpc_cqssc.scrollTop;
						break;
					case "touchend":
						...
						if (spanY > 50 && scrollTops < 5) {
							...
							gpc_cqssc.style.overflow = "hidden";
							gpc_cqssc.style.height = heig + 'px';
							xiedajian(0);
						} else if (spanY < -50) {
							lastTen.style.height = "0";
							gpc_cqssc.removeAttribute("style");
							xiedajian(1);
						}
						break;
				}
	...
	export default {
			...
			return {
				...
				typeList: [{
					...
					},
					{
						...
						'play': 'twoDirect',
						'playMethod': 'direct',
						'text': '每位至少选择1个号码，猜对开奖号码后2位即中100元'
					},
					{
						'id': '2',
						'name': '二星组选',
					...
					},
					{
						...
						'play': 'threeDirect',
						'playMethod': 'direct',
						'text': '每位至少选择1个号码，猜对开奖号码后3位即中1000元'
					},
					{
						'id': '4',
						'name': '三星组三',
						'play': 'threeGroup',
						'playMethod': 'group3',
						'text': '至少选2个号码,猜对开奖号码后3位(顺序不限)即中320元'
					},
					{
						'id': '5',
						'name': '三星组六',
						'play': 'threeGroup',
						'playMethod': 'group6',
						'text': '至少选3个号码,猜对开奖号码后3位(顺序不限)即中160元'
					},
					{
						'id': '6',
						'name': '五星直选',
						'play': 'five',
						'playMethod': 'direct',
						'text': '每位至少选择1个号码，猜对开奖号码后5位即中100000元'
					},
					{
						'id': '7',
						'name': '五星通选',
		...
		mounted() {
			this.gpc_cqssc = document.querySelector("#gpc_cqssc");
		...
		methods: {
			...
			changebg: function() {
				...
				var lastTen = document.querySelector(".last_ten");
				var gpc_hbks = document.querySelector("#gpc_cqssc");
				if (this.bgzhuanhuan) {
					var heig = document.documentElement.clientHeight - document.getElementsByTagName("header")[0].offsetHeight
					lastTen.style.height = "16rem";
					...
					.then(res => {
						...
						if (res.data.status == 200) {
							...
							if (content.code == 100000) {
								var redMiss = {}
								redMiss['dxds'] = {
									'one': [content.data.dxds.onebigMiss, content.data.dxds.onesmallMiss, content.data.dxds.onesingleMiss,
										content.data.dxds.onedualMiss
									],
									'two': [content.data.dxds.twobigMiss, content.data.dxds.twosmallMiss, content.data.dxds.twosingleMiss,
										content.data.dxds.twodualMiss
									]
								}
								redMiss['one'] = []
								redMiss['twoDirect'] = {
					...
					.then(res => {
						...
						if (res.data.status == 200) {
							...
							if (content.code == 100000) {
								...
								EventUtil.listenTouchDirection(document.getElementById("gpc_cqssc"))
							}
						}
					})
					.catch(err => {
			...
			ChoiceType: function() { //点击筛选类型
				if (this.betContent.length > 0) {
					...
					this.gpc_cqssc.removeAttribute("style");
					this.bgzhuanhuan = false;
				}
			},
			...
			setTotalNum: function() { //注数计算
				...
				switch (this.danDallIdx) {
					...
					case 1:
						...
						break;
					case 2:
						totalNum = parseInt(this.getfactorial(this.gelimitNumber) / (this.getfactorial(this.gelimitNumber - 2) * this.getfactorial(
							2)))
						break;
					case 3:
					...
					case 4:
						...
						break;
					case 5:
						totalNum = parseInt(this.getfactorial(this.gelimitNumber) / (this.getfactorial(this.gelimitNumber - 3) * this.getfactorial(
							3)))
						break;
					case 6:
			...
			open_ssqpay: function() { //点击投注
				...
				if (this.betConlength > 0 && this.totalNum < 1) {
					...
					var betContent = {
						...
						playType: this.typeList[this.danDallIdx].play,
						playMethod: this.typeList[this.danDallIdx].playMethod
					};
					if (this.danDallIdx == 8) {
						for (var i = 0; i < 4; i++) {
