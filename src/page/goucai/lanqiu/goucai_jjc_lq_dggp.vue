<template>
	...
	<div>
		<section class="aui-refresh-content">
			...
			<!-- 单关固赔 -->
			<div class="danggp_fm main">
				<template v-for="(item, cindex) in RQSPFlist">
					<div class="countime" :id="'jjc_dggpTime' + cindex" @click="showList(cindex)" data-state="open">
					...
					<div class="jjc_sc_cont jjc_hhgg_cont jjc_hhggl_cont" v-show="showListarr[cindex]">
						<div v-for="(thal, rindex) in item.matchList" class="con" v-if="$options.filters.gamesShow(thal.gameName)">
							...
							<div class="fgr battle">
								<div class="battle_name"><span><i>(客) </i>{{thal.guestTeamName}} </span><span>vs</span><span>
										{{thal.homeTeamName}}<i> (主)</i></span></div>
								<div class="battle_shel">
									<div class="last" @click="showPopup('Popuphhgg','chooseType4',cindex,rindex)" :class="{'el-yellow': thal.genduo}">更多<br />选项</div>
									<ul v-if=thal.playTypeSingle.RFSF class="ul1">
										<li>让分</li>
										<li :class="{'el-yellow': thal.chooseType1[0]}" @click="selectJJC_dggp(cindex,rindex,'chooseType1',0,thal.hhggsp1[0])">{{thal.homeTeamName}}<br /><span>主胜{{thal.hhggsp1[0]}}</span></li>
										...
										<li :class="{'el-yellow': thal.chooseType1[1]}" @click="selectJJC_dggp(cindex,rindex,'chooseType1',1,thal.hhggsp1[1])">{{thal.guestTeamName}}<br />
											<span>客胜{{thal.hhggsp1[1]}}</span></li>
									</ul>
									<ul v-else class="ul1">
										<li>让分</li>
										<li class="zwkf">暂无开放</li>
									</ul>
									<ul v-if=thal.playTypeSingle.SF class="ul2">
										<li>非让分</li>
										<li :class="{'el-yellow':thal.chooseType2[0]}" @click="selectJJC_dggp(cindex,rindex,'chooseType2',0,thal.hhggsp2[0])" class="td_ts">{{thal.homeTeamName}}<br /><span>胜{{thal.hhggsp2[0]}}</span></li>
										<li :class="{'el-yellow':thal.chooseType2[1]}" @click="selectJJC_dggp(cindex,rindex,'chooseType2',1,thal.hhggsp2[1])" class="td_ts">{{thal.guestTeamName}}<br />
											<span>负{{thal.hhggsp2[1]}}</span></li>
									</ul>
									<ul v-else class="ul2">
										<li>非让分</li>
										<li class="zwkf">暂无开放</li>
									</ul>
									<ul v-if=thal.playTypeSingle.DXF class="ul3">
										<li>大小分</li>
										<li :class="{'el-yellow': thal.chooseType3[0]}" @click="selectJJC_dggp(cindex,rindex,'chooseType3',0,thal.hhggsp3[0])">大分<br /><span>{{thal.hhggsp3[0]}}</span></li>
										<li class="td_wd">分数<br /><span>168.5</span></li>
										<li :class="{'el-yellow': thal.chooseType3[1]}" @click="selectJJC_dggp(cindex,rindex,'chooseType3',1),thal.hhggsp3[1]">小分<br />
...
<script>
	...
	export default {
		...
		methods: {
			...
			selectJJC_dggp: function(cindex, rindex, key, i,text) {
